  ...
  return (
    <li className="flex justify-between gap-x-6 py-5">
      ...
      <div className="hidden sm:flex sm:flex-col sm:items-end">
        <p className="text-base leading-6 text-stone-900">{role}</p>
        {phone ? (
          <div className="flex items-center gap-2">
            <PhoneIcon className="h-3 w-3 text-stone-400" aria-hidden="true" />
            <p className="mt-1 text-sm leading-5 text-stone-500">{phone}</p>
          </div>
        ) : (
          role !== "Étudiant" && (
            <div className="flex items-center gap-2">
